...
export const create = ({ bodymen: { body: { email, link } } }, res, next) =>
  User.findOne({ email })
    ...
    .then((reset) => {
      ...
      const content = `
        Hey, ${user.name}.<br><br>
        You requested a new password for your yomi-server account.<br>
        ...
        &mdash; yomi-server Team
      `
      return sendMail({ toEmail: email, subject: 'yomi-server - Password Reset', content })
    })
